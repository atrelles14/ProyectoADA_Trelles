...
en los ciclos de repetición y el tamaño en bytes por funciones*/
...
puntero_matriz3 = new int*[nFilas];  //Reservando memoria para las filas    2 OE
    for(int i=0;i<nFilas;i++){                  // 4 OE
        puntero_matriz3[i] = new int[nCol]; //Reservando memoria para las columnas      //2 OE
...
cout<<"\nDigite los elementos de la tercera matriz:\n";             //1 OE
    ...
    cout<< "Ingresaste las matrices durante"<<float(tiempo_req)/CLOCKS_PER_SEC<<" segundos. "; //3 OE
}
...
void imprimirMatriz(int **puntero_matriz, int nFilas, int nCol){        
    ...
    for(int i=0; i<nFilas;i++){                         //4 OE
        for(int j=0;j<nCol;j++){                        //4 OE
            cout<<*(*(puntero_matriz+i)+j)<<" "; //puntero_matriz 3 OE
}
    cout<<"\n";                                         //1 OE
}
